import type { Item } from '@shridhar/shared';
import type { Repo } from './store/types';

/**
 * Starter list. The first four rows are read off the shop's paper slip dated 9/3/26 -- the
 * Kannada handwriting is a best guess, so CONFIRM these four names with the shopkeeper before
 * going live. Their rates are exact (550/5 = 110, 615/5 = 123, 50, 155), which means billing
 * 5 + 5 + 1 + 1 of them reproduces that slip's total of 1370 -- useful for a demo.
 *
 * Everything after them is ordinary kirana stock at placeholder rates, purely so the app has
 * something to show on day one. The shopkeeper edits all of it from the Items page.
 */
...
/** Only ever fills an empty shop. It must never overwrite rates the shopkeeper has edited. */
export async function seedIfEmpty(repo: Repo): Promise<void> {
  const inserted = await repo.seedItems(SEED_ITEMS);
  if (inserted > 0) console.log('[seed] inserted ' + inserted + ' starter items');
}
